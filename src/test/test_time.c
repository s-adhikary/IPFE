...
{
	// Declarate variables
	__attribute__((aligned(32))) uint32_t mpk[SIFE_L+1][SIFE_NMODULI][SIFE_N];
	__attribute__((aligned(32))) uint32_t msk[SIFE_L][SIFE_NMODULI][SIFE_N];
	__attribute__((aligned(32))) uint32_t c[SIFE_L+1][SIFE_NMODULI][SIFE_N];
	__attribute__((aligned(32))) uint32_t sk_y[SIFE_NMODULI][SIFE_N];
	//uint32_t d_y[SIFE_NMODULI][SIFE_N];

	__attribute__((aligned(32))) int32_t m[SIFE_L];
	__attribute__((aligned(32))) int32_t y[SIFE_L];
...
int test_rlwe_sife_mat_vec()			/*Only matrix-vector multiplication*/
{
	...
	__attribute__((aligned(32))) int32_t m[SIFE_L][SIFE_N];
	__attribute__((aligned(32))) int32_t y[SIFE_L];
	int32_t res_vec[SIFE_N];
	...
	for(i = 0; i < N_TESTS; i++) {
		...
		*/
		...
		for(k=0;k<SIFE_N;k++){
			if(mpz_cmp_si(dy[k], res_vec[k])!=0){
				printf("--ERROR---\n");
				return 0;
			}
		}
